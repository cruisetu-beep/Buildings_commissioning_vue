...
/* D04（CR0023）。判据 powerRatio = 冷机过渡季日电耗 ÷ 冷机盛夏日电耗，
   ≥ 业态阈值触发（本条实测三窗口阈值恒为 0.8）。

   ⚠⚠ 同一 ruleId 下有两种 payload 形态，靠 metrics.gate 区分：
        常规    {powerRatio, threshold, rFanChiller, passed}
        除零门控 {gate:"zeroDivisionPass", passed:true}
      门控形态里 dayB 是 dayA 的**副本**（日期、标签、两列数值逐个相同），
      判据没算。此时 passed=true 的意思是「放行」而非「合格」，
      而 category 仍写「正常」——若走 normal 文案就是把「未判定」说成
      「已检查、未落入触发区间」。故本常量单列 undetermined 分支，
      窗口条标记与结论卡色调同步退到「待核查」那一态。

   ⚠ 触发窗口的分母可能是个「冷机整天没开」的日子。实测 A013：
        窗口1  04-01 vs 08-09(周六)  冷机 25 → 725   ⇒ 3.5%  正常
        窗口3  04-02 vs 08-23(周六)  冷机 30 → 925   ⇒ 3.2%  正常
        窗口2  04-13 vs 08-24(周日)  冷机 30 →  25   ⇒ 120%  触发
      窗口2 的盛夏参照日冷机仅 25 kWh（另两个盛夏日是 725/925），
      整栋楼那天风机也只有 1076（另两天 3504/3495），是周日基本没运行。
      分子 30 与另两窗口的 25/30 几乎相同——**触发完全由分母产生**。
      换任一个正常盛夏日作分母：30/725=4.1%、30/925=3.2%，都判正常。
      属六·G（选窗撞上「设备整天没开」）第 10 次，但前 9 次撞的是被测日，
      这次撞的是对照日/分母，判据无分母下限保护。已挂问题清单。
      文案因此退到中性，并在触发分支里点出分母的绝对量级供读者自行判断。

   ⚠ rFanChiller 算了但**不参与判据**（判定准则表只有 powerRatio 一条）。
      resultMd 步骤2 只给了 R_fan 的公式，反而没给判据指标 powerRatio 的公式。
      本常量把它列为参考值（kind: stated），不给 ✓/✕。

   ⚠ 阈值随业态变（judgmentStandard：AA/BH 50%、BB 80%、其余 55%~78%），
      但 payload 不带业态编码，取的哪一档无法核对。文案只写实际阈值数值。

   ⚠ meteorology 只有 {variable, note, unit}，没有任何数值，也没有
      conditionPassed——不能像 C08 那样设气象前提步骤。note 是新字段，
      常规形态写「4月 h_out<45」、门控形态写「除零门控」。 */
const D04 = {
  narrative: {
    triggered:
      "{dayA}（{labelA}）与 {dayB}（{labelB}）相比，冷机日电耗为 " +
      "<b>{chillerA} kWh</b> 与 <b>{chillerB} kWh</b>，比值 <b>{powerRatio}</b>，" +
      "<b>落入触发区间</b>（≥ {thresholdAbs}）。" +
      "留意作为分母的盛夏日冷机为 {chillerB} kWh —— 该日冷机若本就基本未运行，" +
      "比值会被分母抬高，此时该比值不反映过渡季的控制水平。",
    normal:
      "{dayA}（{labelA}）与 {dayB}（{labelB}）相比，冷机日电耗为 " +
      "<b>{chillerA} kWh</b> 与 <b>{chillerB} kWh</b>，比值 <b>{powerRatio}</b>，" +
      "未落入触发区间（≥ {thresholdAbs}）。",
    /* 门控：不复述 category 的「正常」，如实说没算 */
    undetermined:
      "本窗口<b>未做判定</b>。冷机日电耗为 0，比值的分母为零，后端以除零门控直接放行" +
      "（<code>gate = zeroDivisionPass</code>），并未计算过渡季与盛夏的冷机功耗比。" +
      "同时该窗口的两日是<b>同一天</b>（{dayA}），盛夏对照日未取到，" +
      "能耗两列为相同数值，故不出图。风机日电耗 {fanA} kWh 可在下方原始数据中查看。",
  },
  title: {
    triggered: "冷机过渡季/盛夏功耗比落入触发区间",
    normal: "冷机过渡季/盛夏功耗比未落入触发区间",
    undetermined: "未做判定：分母为零，除零门控放行",
  },
  steps: [
    {
      kind: "stated",
      what: "取数据",
      sub: "冷热站 U2A00 的日累计电耗，过渡季与盛夏各取一日",
      val: "{labelA} {chillerA} kWh · {labelB} {chillerB} kWh",
      req: "—",
    },
    {
      kind: "test",
      what: "冷机功耗比 E_trans / E_summer",
      sub: "过渡季 ÷ 盛夏。阈值随业态编码变，payload 未带编码，此处显示实际取值",
      val: "{powerRatio}",
      req: "≥ {thresholdAbs}",
      key: "pr",
    },
    {
      kind: "stated",
      what: "送风-冷机能耗比 R_fan（参考，不参与判定）",
      sub: "(U2B01 + U2B02) ÷ U2A00，后端算出但不在判定准则内",
      val: "{rFan}",
      req: "—",
    },
  ],
  foot: {
    triggered: "判据满足 → 判定为 <b>目标调适</b>",
    normal: "判据未满足 → 判定为 <b>正常</b>",
    undetermined: "判据未计算 → 后端以<b>除零门控</b>放行，本窗口不构成合格结论",
  },
  /* 手册 D04「物理原理」原文 */
  causes: [
    "新回风焓值转换控制失效，过渡季不切换到全新风/最大新风免费供冷",
    "冷机在室外焓值已低于室内时仍按夏季模式启停，制冷量被无谓消耗",
    "新风阀执行器卡死或最小新风比锁死，免费供冷路径在物理上不可用",
  ],
  readHint: {
    slope:
      "纵轴是冷机日电耗的绝对值（kWh）。图上只画冷机——判据只由它的两日值决定，" +
      "风机不参与（两者量级差 149~188 倍，同轴会把冷机压成贴底的一条线），" +
      "风机数值在「数据」页签里。留意盛夏那一天的绝对值：它是比值的分母，" +
      "若该日冷机本就接近零，比值会被抬高到与过渡季控制水平无关的量级。",
  },
};

/* BF-S1（CR0044）。S 系体育场馆，E 类双曲线。
   判据 gap = (E_L − E_H)/E_H < 30% 触发（两天差距小＝低负荷日没卸载）。

   ⚠ resultMd 里印的公式是 (E_H − E_L)/E_H，与后端实现的分子顺序相反，
      三个窗口无一例外。两种算法的判定结果差别很大：按 resultMd 的公式，
      三个窗口的 gap 全为负、全部 < 30%、全部触发；按实现只触发窗口1。
      **本常量按实现接入**，页面不加提示（已确认）。已挂问题清单。

   ⚠ 判据只测差距的大小、不约束方向。实测三窗口 E_L 全部大于 E_H：
        窗口1  E_H=51.0  E_L=61.6  低/高 = 1.21×  gap= 20.8%  触发
        窗口2  E_H=24.8  E_L=54.4  低/高 = 2.19×  gap=119.4%  正常
        窗口3  E_H=37.9  E_L=57.4  低/高 = 1.51×  gap= 51.4%  正常
      即低负荷日反而比高负荷日耗得多，而差距越大越判正常。
      文案因此只陈述数值与落点，不写「已卸载」「运行正常」这类解释性结论；
      E_H / E_L 的绝对值在「取数据」行照常显示，读者自行判断。

   ⚠ eH / eL 是 hourlyProfiles 积分的 4.00 倍（六·B，实测 4.01/3.99/4.01）。
      D05、AA-S2 靠改用积分绕开，BF-S1 绕不开——判据就建在 eH / eL 上。
      gap 是比值，4 倍在分子分母对消，结论不受影响。
      **不要「顺手修正」eH / eL，那会把判据搞坏。**

   ⚠ 窗口3 的 gap 复现不上：(57.4−37.9)/37.9 = 0.514512，后端给 0.5144，
      差 1.1e-4（窗口1、2 都在 5e-5 内）。疑似 eH/eL 的显示值被舍入过。
      前端只显示后端的 gap，不重算。

   ⚠ requiredNodeTypes 是 U2A01/U2A02/U2A04（冷冻泵、冷却泵、冷却塔），
      而 resultMd 写「数据来源：U2000 空调系统总电耗逐时数据」——
      用全楼空调总电耗判「冷却系统未卸载」，六·C 家族。文案只说
      「空调系统总电耗」，不冒称是冷却系统三节点。

   ⚠ 曲线归属：Σworkday×4 ≈ eH，故 workday ↔ dateFrom ↔ 高负荷日。
      标签取 selectionReason 切段后的各自前缀，不能套 DayPairResidual
      那个整串取前缀的正则（会把「高负荷」贴到低负荷曲线上）。 */
const BF_S1 = {
  narrative: {
    triggered:
      "{wdName} {wdDate} 与 {holName} {holDate} 的空调系统总电耗分别为 " +
      "<b>{eH} kWh</b> 与 <b>{eL} kWh</b>，两日差距 gap = <b>{gap}</b>，" +
      "<b>落入触发区间</b>（< {gapThreshold}）。两条逐时曲线的形状见下图。",
    normal:
      "{wdName} {wdDate} 与 {holName} {holDate} 的空调系统总电耗分别为 " +
      "<b>{eH} kWh</b> 与 <b>{eL} kWh</b>，两日差距 gap = <b>{gap}</b>，" +
      "未落入触发区间（< {gapThreshold}）。两条逐时曲线的形状见下图。",
  },
  /* 描述性表述：只说落点，不写「已卸载」这类解释 */
  title: {
    triggered: "高/低负荷日差距落入触发区间",
    normal: "高/低负荷日差距未落入触发区间",
  },
  steps: [
    {
      kind: "stated",
      what: "取数据",
      sub: "两日的空调系统总电耗（U2000 逐时数据日累计）",
      val: "{wdName} {eH} kWh · {holName} {eL} kWh",
      req: "—",
    },
    {
      kind: "test",
      what: "两日差距 gap",
      sub: "低负荷日与高负荷日之差 ÷ 高负荷日。判据为反向：差距越小越指向未卸载",
      val: "{gap}",
      req: "< {gapThreshold}",
      key: "gap",
    },
  ...
  /* 手册 BF-S1「物理原理」原文 */
  causes: [
    "场馆冷却系统按赛事日满负荷整定后未随场次日历回调，非赛事日沿用同一套参数",
    "冷却塔风机与冷却泵缺乏台数控制，无论负荷高低均全台投运",
    "空调系统未接入场馆运营日历，无赛事日仍按预设时间表启停",
  ],
  readHint: {
    day:
      "两条线是同一套设备在高负荷日与低负荷日的逐时功率。" +
      "判据看的是两天的日累计电耗差距，而不是曲线形状——" +
      "但形状能看出差距出在哪些时段：若两条线在非使用时段仍贴在一起，" +
      "说明设备在这些小时里没有随负荷回落。" +
      "纵轴的逐时功率与「取数据」行的日电耗之间存在固定倍率差，" +
      "两者不可直接相加核对。",
  },
};
...
/* BH-S1（CR0046）。S 系教育建筑，E 类双曲线。
   判据 ratio = vacMean / termMean > 60% 触发（假期能耗没降下来）。

   ⚠⚠ **图上两条曲线是代表日，判据用的却是两个均值，两者对不上。**
      Σworkday×4 = 893.72 vs termMean 912.89（倍率 4.086，差 2.1%）
      Σholiday×4 = 1065.72 vs vacMean 1064.16（倍率 3.994）
      说明 termMean 多半是「学期全部日期的日均」，而 hourlyProfiles.workday
      只是其中一个代表日。故图上另画两条均值水平线，
      **文案必须明说判据用的是均值，不能说「图上这两天的比值就是判据」。**
      均值换算到逐时功率轴需 ÷96（4 倍 × 24 小时）：9.51 / 11.09 kW。
      该换算依赖六·B 的 4 倍缺陷成立，后端修了要同步改。

   ⚠ 代表日不在 sWindow 里。sWindow 的 dateFrom/dateTo 是全年统计窗
      （2025-01-01 ~ 2025-12-31，windowType「段」），真正的代表日在
      metrics.vacRep / termRep。照搬 BF-S1 那套会把日期标成 01-01 / 12-31。

   ⚠ 阈值三处不一致：外层 thresholdValue 写「假期/学期≥80%」，
      metrics.threshold = 0.6，resultMd 判定准则写「> 60.0%」。
      按 metrics 的 60% 实现。本窗口 ratio=116.6%，两个阈值都会触发，
      结论不受影响，但需后端澄清。六·I 家族。

   ⚠ 代表日的星期不对等：假期代表日 2025-08-16 是**周六**，学期代表日
      2025-11-19 是**周三**。学校假期里周末与平日无区别，学期中周六通常
      要降载——拿假期的周六比学期的周三会让比值偏高。本窗口 116.6% 离
      60% 很远、不影响结论，但选窗逻辑需复核。六·G 家族。

   ⚠ 只有一个窗口且为触发，**无正常样本**，判定方向未经反向验证。

   ⚠ 曲线归属已核对：workday ↔ 学期（termMean）、holiday ↔ 假期（vacMean）。 */
const BH_S1 = {
  narrative: {
    triggered:
      "假期日均空调总电耗 <b>{vacMean} kWh</b>，学期日均 <b>{termMean} kWh</b>，" +
      "两者之比 <b>{ratio}</b>，<b>落入触发区间</b>（> {ratioThreshold}）。" +
      "下图是两侧各取一个代表日的逐时曲线（{holName} {holDate}、{wdName} {wdDate}）；" +
      "判据用的是上述两个均值，不是这两天本身，两条虚线即均值折算到逐时功率后的位置。",
    normal:
      "假期日均空调总电耗 <b>{vacMean} kWh</b>，学期日均 <b>{termMean} kWh</b>，" +
      "两者之比 <b>{ratio}</b>，未落入触发区间（> {ratioThreshold}）。" +
      "下图是两侧各取一个代表日的逐时曲线（{holName} {holDate}、{wdName} {wdDate}）；" +
      "判据用的是上述两个均值，不是这两天本身，两条虚线即均值折算到逐时功率后的位置。",
  },
  title: {
    triggered: "假期与学期能耗比落入触发区间",
    normal: "假期与学期能耗比未落入触发区间",
  },
  steps: [
    {
      kind: "stated",
      what: "取数据",
      sub: "假期与学期各自的日均空调系统总电耗（U2000）",
      val: "假期 {vacMean} kWh · 学期 {termMean} kWh",
      req: "—",
    },
    {
      kind: "test",
      what: "假期/学期能耗比",
      sub: "假期日均 ÷ 学期日均。假期本应显著低于学期，比值偏高即指向未卸载",
      val: "{ratio}",
      req: "> {ratioThreshold}",
      key: "vacRatio",
    },
    {
      kind: "stated",
      what: "代表日（仅供看曲线形态，不参与判定）",
      sub: "两条曲线各取一日；判据用的是上一行的均值，与这两天不完全一致",
      val: "{holName} {holDate} · {wdName} {wdDate}",
      req: "—",
    },
  ],
  foot: {
  ...
  /* 手册 BH-S1「物理原理」原文 */
  causes: [
    "空调系统未接入校历，寒暑假期间仍按学期作息表启停",
    "冷水主机与末端风机无假期模式，房间无人时仍维持设计温湿度",
    "假期集中安排的场馆开放或培训占用少数区域，却整栋楼投运冷源",
  ],
  readHint: {
    day:
      "两条实线是假期与学期各取的一个代表日的逐时功率，两条虚线是判据真正用的" +
      "日均能耗折算到逐时功率后的位置——虚线与对应实线的平均高度不完全重合，" +
      "因为均值取自各自区间的全部日期，而实线只是其中一天。" +
      "若假期曲线在白天时段与学期贴近甚至更高，说明假期没有随使用需求降载。",
  },
};
